...
class Edition(RemoteModel):
    ...
    def automaticGenerationQueue(self):
        """Queues the generation of this edition"""
        if self.auto_generation_config is None:
            raise ValueError("No auto generation configuration set for this edition")
        if self.actively_generated_from is not None:
            raise ValueError("This edition doesnt have a parent edition to be generated from")
        if self.auto_generation_config.automatically_regenerate:
            async_task('archive_backend.jobs.start_autogeneration_for', pkStringList([self]),
               task_name=("Autogenerating edition: " + self.title)[:100])
        self.auto_generation_config.queueGeneration(self)
